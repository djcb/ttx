...
#define PROVIDER_ID TTX_PROVIDER_NOS_TELETEKST
...
on_got_page (TTXHTTPStatus hstatus, const char *path, CBData *cbdata)
{
	...
	links_uri = g_strdup_printf (
		"http://nos.nl/data/teletekst/xml/P%u_%02u.xml",
		cbdata->page, cbdata->subpage);
...
void
ttx_provider_nos_teletekst_retrieve (unsigned page, unsigned subpage,
				     const char *dir,
				     TTXProviderResultFunc func,
				     gpointer user_data)
{
	...
	page_uri  = g_strdup_printf (
		"http://nos.nl/data/teletekst/gif/P%u_%02u.gif",
		page, subpage);
